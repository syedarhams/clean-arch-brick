...
```dart
import 'package:muebly/features/customer/user_profile/domain/repositories/user_profile_repository.dart';
...
Cubit with repository injection:
```dart
import 'package:muebly/exports.dart';
import 'package:muebly/features/customer/user_profile/domain/repositories/user_profile_repository.dart';
import 'package:muebly/features/customer/user_profile/presentation/cubit/state.dart';
...
| `feature_path` | The path under `lib/features/` (user type) | `customer` | `business` |
...
Install Mason CLI globally (one-time):

```bash
dart pub global activate mason_cli
```

Add `pub-cache/bin` to your PATH (one-time). Add this line to your `~/.zshrc` or `~/.bashrc`:
...
### Option A: Use from Git (for teammates / cross-project)
...
2. Add the brick to your `mason.yaml`:
...
### Option B: Use locally (already set up in this project)

The brick is already registered in `mason.yaml` via local path:
...
```bash
mason make feature --output-dir lib/features/customer
```

It will ask you for:
1. Feature name (e.g. `user_profile`)
2. Package name (defaults to `muebly`)
3. User type path (defaults to `customer`)
...
```bash
# Skip existing files
mason make feature --output-dir lib/features/customer --on-conflict skip

# Overwrite existing files
mason make feature --output-dir lib/features/customer --on-conflict overwrite
```
